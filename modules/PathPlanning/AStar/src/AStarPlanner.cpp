...
AStarPlanner::AStarPlanner(std::vector<double> o_x,
...
std::shared_ptr<Path> AStarPlanner::Plan(double start_x, double start_y, double goal_x, double goal_y)
{
    ...
    while (true) 
    {
        ...
        for (int i=0; i<motion.size(); i++)
        {
            ...
            Node* new_node = new Node(node->x_+motion[i].x_,
                                      node->y_+motion[i].y_,
                                      path_cost[node->x_][node->y_]+motion[i].cost_+CalcHeuristic(goal_node, node),
                                      node);
...
std::shared_ptr<Path> AStarPlanner::CalcFinalPath(Node* goal_node)
{
...
double AStarPlanner::CalcHeuristic(Node* n1, Node* n2, double w)
{
    return w * hypot(n1->x_-n2->x_, n1->y_-n2->y_);
}

double AStarPlanner::CalcGridPosition(int node_index, double min_position)
{
...
int AStarPlanner::CalcXYIndex(double position, double min_position)
{
    return round((position-min_position)/resolution_);
}

double AStarPlanner::CalcGridIndex(Node node)
{
    return (node.y_-occupancy_map_.min_y_)*occupancy_map_.x_width_+(node.x_-occupancy_map_.min_x_);
}

bool AStarPlanner::VerifyNode(Node* node)
{
...
std::shared_ptr<OccupancyMap> AStarPlanner::CreateOccupancyMap(std::vector<double> o_x, std::vector<double> o_y)
{
...
std::vector<Node> AStarPlanner::GetMotionModel()
{
    // dx, dy, cost
